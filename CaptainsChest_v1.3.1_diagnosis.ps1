...
function Write-FleetDiagnosisSummary {
    ...
    if ($Reachable -eq $script:WindroseServiceEndpoints.Count) {
        ...
        Write-Line 'If the game still fails, the issue is likely local to the PC, Steam, or the game client.'
        Write-Line ''
        Write-Line 'Next steps:'
        Write-Line '  - Restart Windrose and Steam'
        Write-Line '  - Verify game files'
        Write-Line '  - Check local firewall/AV prompts'
        Add-Finding -Status 'PASS' -Check 'Diagnosis' -Details 'All Windrose backend services reachable. Likely a local client, Steam, or game-side issue if problems continue.'
        return
    }

    if ($AnyDnsSpoof) {
        Write-TaggedLine -Status 'FAIL' -Text 'Your DNS is returning fake or filtered answers for one or more Windrose domains.'
        Write-Line 'This usually means a VPN, parental controls, safe browsing, or a custom DNS filter is intercepting the game.'
        Write-Line ''
        Write-Line 'Next steps:'
    ...
    if ($AnyIspBlock) {
        if ($DetectedCulprit) {
            Write-TaggedLine -Status 'FAIL' -Text ("Your ISP or ISP-provided router is likely blocking Windrose services. Most likely culprit: {0} -> {1}." -f $DetectedCulprit.Name, $DetectedCulprit.Feature)
        ...
        Write-Line 'The game may show N/A for Connection Services because DNS works on Google DNS but not on your current network path.'
        Write-Line ''
        Write-Line 'Next steps:'
        ...
        Write-Line ''
        Write-TaggedLine -Status 'INFO' -Text 'Fallback option: if Direct IP hosting works for your crew, it bypasses Connection Services entirely and uses port 7777.'
    ...
    if ($AnyIpv6Issue) {
        ...
        Write-Line 'The domains resolve, but the game may still fail because the client does not handle IPv6-only paths correctly.'
        Write-Line ''
        Write-Line 'Next steps:'
        Write-Line '  - Keep IPv6 enabled, but set Windows to prefer IPv4'
        Write-Line '  - Reboot after applying the registry change shown above'
        Add-Finding -Status 'FAIL' -Check 'Diagnosis' -Details 'IPv6 prioritization issue detected for Windrose services.'
        return
    }
    ...
    if ($HasPartialReachability) {
        if (-not $Stun3478Reachable) {
            if ($DetectedCulprit) {
                Write-TaggedLine -Status 'WARN' -Text ("Some Windrose services work, but peer-to-peer signaling is blocked. Likely cause: {0} -> {1}." -f $DetectedCulprit.Name, $DetectedCulprit.Feature)
                Write-Line ("Toggle path: {0}" -f $DetectedCulprit.Toggle)
            } else {
                Write-TaggedLine -Status 'WARN' -Text 'Some Windrose services work, but peer-to-peer signaling on port 3478 is blocked.'
            }
            Write-Line 'This usually means the game can see part of the backend, but cannot complete the connection path needed for players to join each other.'
            Write-Line ''
            Write-Line 'Next steps:'
            ...
            Add-Finding -Status 'WARN' -Check 'Diagnosis' -Details 'Partial reachability with port 3478 blocked. Likely ISP/router security or peer-to-peer signaling failure.'
        } else {
            Write-TaggedLine -Status 'WARN' -Text 'Some Windrose endpoints are reachable and some are not.'
            Write-Line 'This points to a regional backend issue, routing problem, or selective ISP filtering.'
            Write-Line ''
            Write-Line 'Next steps:'
            ...
            Add-Finding -Status 'WARN' -Check 'Diagnosis' -Details 'Partial reachability across Windrose endpoints. Possible regional outage, routing issue, or selective ISP filtering.'
        }
        return
    }

    Write-TaggedLine -Status 'FAIL' -Text 'All Windrose endpoints are unreachable from this machine.'
    Write-Line 'This is either a full backend outage, a severe local/network block, or a broken internet path.'
    Write-Line ''
    Write-Line 'Next steps:'
